...
func NewGrpcTensorflowServer(predictor *v1.PredictorSpec, client client.SeldonApiClient, serverUrl *url.URL, namespace string) *GrpcTensorflowServer {
	return &GrpcTensorflowServer{
		Client:    client,
		predictor: predictor,
		Log:       logf.WithName("SeldonGrpcApi"),
		ServerUrl: serverUrl,
		Namespace: namespace,
	}
}
...
func (g *GrpcTensorflowServer) execute(ctx context.Context, req proto.Message, method string) (payload.SeldonPayload, error) {
	...
	seldonPredictorProcess := predictor.NewPredictorProcess(ctx, g.Client, logf.WithName(method), g.ServerUrl, g.Namespace, md)
...
func (g *GrpcTensorflowServer) GetModelMetadata(ctx context.Context, req *serving.GetModelMetadataRequest) (*serving.GetModelMetadataResponse, error) {
	seldonPredictorProcess := predictor.NewPredictorProcess(ctx, g.Client, logf.WithName("GrpcGetModelMetadata"), g.ServerUrl, g.Namespace, grpc.CollectMetadata(ctx))
	reqPayload := payload.ProtoPayload{Msg: req}
...
func (g *GrpcTensorflowServer) GetModelStatus(ctx context.Context, req *serving.GetModelStatusRequest) (*serving.GetModelStatusResponse, error) {
	seldonPredictorProcess := predictor.NewPredictorProcess(ctx, g.Client, logf.WithName("GrpcGetModelStatus"), g.ServerUrl, g.Namespace, grpc.CollectMetadata(ctx))
	reqPayload := payload.ProtoPayload{Msg: req}
